...
账号列表在状态后显示独立的 **Credit** 列，账号详情也可查看个人 Credit 余额。余额通过 Cline 官方 `/users/me` 和 `/users/{id}/balance` 接口查询，按官网网页的规则将 microcredits 除以 1,000,000 换算为 Credit（例如 `499962 → 0.499962`，四位小数显示 `0.5000`），点击金额可刷新；仅加载当前页，最多并发查询 3 个账号，成功结果缓存 1 分钟。失败不会显示成零，已有余额会保留并标记刷新失败。Credit 与模型每日免费额度、订阅用量是不同概念。
...
页面直接列出所有已添加模型，未设置上游的默认显示 **自动**。每页默认显示 10 个，可切换为 20 / 50 / 100 个。点击模型所在行的 **配置**，在弹窗中 **探测渠道**、勾选渠道并保存；清除配置后，已添加的模型仍保留在列表中。模型库之外的历史配置也会保留并单独标注，方便管理别名与重定向。
...
一键 **探测渠道** 会向网关发一到两次小请求，显示实际命中的渠道与是否与你的钉住一致。
网关支持渠道枚举时返回完整清单；若忽略枚举筛选，则回读 planner 响应元数据中的命中/候选渠道，明确标注清单可能不完整、严格钉住未验证。
这些元数据候选不会用于「排除渠道」的白名单换算。枚举筛选被忽略时仍可能生成回答并消耗 token。探测结果会缓存到账号池文件里。
...
|---|---|
| `latest` | 默认分支的最新构建 |
| `1.7.10` | 当前源码版本号（`cmd/cline-proxy/version.go`）；镜像构建时读取该值作为版本标签 |
| `sha-<commit>` | 对应某次提交 |

镜像标签直接来自源码里的版本号，所以拉取任意一个版本标签就能固定到具体那一版：

```bash
docker pull ghcr.io/qcenjoyll/cline-proxy:1.7.10   # 对应版本发布后可固定使用
