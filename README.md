...
- **中文描述**：模型描述由服务端对照翻译为中文；上游若新增或改写描述，未命中对照时回退显示原文，不会丢内容
...
|---|---|
| `latest` | 默认分支的最新构建 |
| `1.3.8` | 源码版本号（`cmd/cline-proxy/version.go`）。**每次提交都会随版本号自动更新** |
| `sha-<commit>` | 对应某次提交 |

镜像标签直接来自源码里的版本号，所以拉取任意一个版本标签就能固定到具体那一版：

```bash
docker pull ghcr.io/qcenjoyll/cline-proxy:1.3.8   # 固定版本
