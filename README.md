...
- **全部模型（默认折叠）**：列表底部还有一个默认收起的折叠区块，展示 `api/v1/ai/cline/models` 的全部模型（目前 400+ 条）。卡片格式与交互和上方分组完全一致，只是展开时才向服务端取数
...
|---|---|
| `latest` | 默认分支的最新构建 |
| `1.4.0` | 源码版本号（`cmd/cline-proxy/version.go`）。**每次提交都会随版本号自动更新** |
| `sha-<commit>` | 对应某次提交 |

镜像标签直接来自源码里的版本号，所以拉取任意一个版本标签就能固定到具体那一版：

```bash
docker pull ghcr.io/qcenjoyll/cline-proxy:1.4.0   # 固定版本
