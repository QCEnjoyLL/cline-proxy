...
页面直接列出所有已添加模型，未设置上游的默认显示 **自动**。支持按模型 ID 实时搜索（忽略大小写），搜索后按匹配结果分页。每页默认显示 10 个，可切换为 20 / 50 / 100 个。点击模型所在行的 **配置**，在弹窗中 **探测渠道**、勾选渠道并保存；清除配置后，已添加的模型仍保留在列表中。模型库之外的历史配置也会保留并单独标注，方便管理别名与重定向。
